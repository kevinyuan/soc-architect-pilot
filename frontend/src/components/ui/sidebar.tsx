...
const SidebarProvider: React.FC<SidebarProviderProps> = (
  ...
  // Use prop for open state if provided and controlled, otherwise use internal state
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((current: boolean) => boolean)) => {
      ...
      // Only manage cookie if not externally controlled by onOpenChange for open state
  ...
  const toggleSidebar = React.useCallback(() => {
    if (isMobile) {
      ...
      // If 'open' is controlled, onOpenChange should handle toggling.
      // If not, use internal setOpen.
...
interface SidebarSpecificProps {
  side?: "left" | "right";
...
type SidebarProps = SidebarSpecificProps & Omit<React.ComponentProps<"div">, keyof SidebarSpecificProps>;
...
const Sidebar = React.forwardRef<HTMLDivElement, SidebarProps>(
  (
    ...
    if (isPropsOpenDefined) {
      currentOpen = propsOpen; // Prioritize direct prop
    } else {
      currentOpen = isMobile ? contextOpenMobile : contextOpen; // Fallback to context
    }
    
    // Determine the effective onOpenChange handler
    let effectiveOnOpenChange = propsOnOpenChange;
    if (!isPropsOpenDefined && !propsOnOpenChange) { // If instance is not controlled for open/onOpenChange
        // effectiveOnOpenChange = isMobile ? contextSetOpenMobile : contextSetOpen;
        // For offcanvas, if not controlled, it means it is likely controlled by a header button
        // So, we don't want the Sheet's internal onOpenChange (like overlay click) to call contextSetOpen for the *left* panel.
        // If propsOnOpenChange is not provided, the Sheet should not attempt to call it.
        // The Sheet itself will use its own internal `onOpenChange` for its Radix Dialog.
        // We only care about `propsOnOpenChange` if the consuming component wants to react to changes.
    }
    ...
    if (isMobile) {
      return (
        <Sheet
          open={currentOpen}
          onOpenChange={effectiveOnOpenChange || contextSetOpenMobile} // Sheet needs an onOpenChange
        >
          <SheetContent
    ...
    // Desktop logic
    if (collapsible === "offcanvas" && !currentOpen) { 
      return (
        <div
          ref={ref}
          className={cn(className, "hidden")} // Ensure hidden is applied effectively, placed after className for twMerge
    ...
    if (variant === "floating" || variant === "inset") {
      ...
      finalWidthClass = isIconModeActive
        ...
        : "w-[calc(var(--this-sidebar-width,var(--sidebar-width))_+_theme(spacing.4))]";
    } else { 
      ...
      finalWidthClass = isIconModeActive
        ...
        : "w-[var(--this-sidebar-width,var(--sidebar-width))]";
    }
